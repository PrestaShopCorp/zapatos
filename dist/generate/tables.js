...
const relationsInSchema = async (schemaName, transforms, queryFn) => {
...
const columnsForRelation = async (rel, schemaName, transforms, queryFn) => {
    const { rows } = await queryFn({
        text: rel.type === 'mview'
            ? `
        SELECT
...
config, queryFn) => {
    const rows = await columnsForRelation(rel, schemaName, config.nameTransforms, queryFn), selectables = [], JSONSelectables = [], whereables = [], insertables = [], updatables = [];
    rows.forEach(row => {
        var _a;
