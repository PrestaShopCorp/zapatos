...
exports.nullTransforms = {
    ts: {
        fromPgToTs: utils_1.noop,
        fromTsToPg: utils_1.noop,
    },
    pg: {
        fromPgToTs: utils_1.noop,
        fromTsToPg: utils_1.noop,
    ...
    pg: {
        ...
        namedColumnsJSON: (columns) => sql `jsonb_build_object(${(0, utils_1.mapWithSeparator)(columns, sql `, `, (c) => sql `${param(c)}::text, ${camelToSnakeFn(c)}`)})`,
...
const config = {
    ...
    nameTransforms: exports.nullTransforms,
};
/**
...
const setConfig = (newConfig) => {
    ...
    Object.assign(config, newConfig, // might be a lie now, but we're about to fix that
    nameTransforms === false
        ? { nameTransforms: exports.nullTransforms }
        : nameTransforms === true
            ? { nameTransforms: exports.snakeCamelTransforms }
            : {});
};
...
class SQLFragment {
    constructor(literals, expressions) {
         ...
         */
        this.runResultTransform = (qr) => {
            const { fromPgToTs } = config.nameTransforms.ts, { rows } = qr;
            if (fromPgToTs === utils_1.noop)
                return rows;
            const namesMap = Object.fromEntries(qr.fields.map((f) => [f.name, fromPgToTs(f.name)]));
            for (let i = 0, len = rows.length; i < len; i++) {
                rows[i] = Object.fromEntries(Object.entries(rows[i]).map(([k, v]) => [namesMap[k], v]));
            }
            return rows;
        };
        ...
        this.run = async (queryable, force = false) => {
            var _a;
            const query = this.compile(), { queryListener, resultListener } = (0, exports.getConfig)(), txnId = (_a = queryable._zapatos) === null || _a === void 0 ? void 0 : _a.txnId;
            if (queryListener)
                ...
                if (columnNames.length) {
                    ...
                    for (let i = 0, len = columnNames.length; i < len; i++) {
                        ...
                        if (columnValue instanceof SQLFragment) {
                            ...
                            result.text += ` = `;
                            this.compileExpression(columnValue instanceof ParentColumn
                                ? columnValue
                                : new Parameter(columnValue), result, parentTable, columnName);
                        }
                    }
                    result.text += ")";
                }
