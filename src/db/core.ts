...
const snakeToCamelFn = (s: string) =>
    s.replace(/_[a-z]/g, (m) => m.charAt(1).toUpperCase()),
  camelToSnakeFn = (s: string) =>
...
export const nullTransforms: NameTransforms = {
    ts: {
      fromPgToTs: noop,
      fromTsToPg: noop,
    },
    ...
    pg: {
      ...
      namedColumnsJSON: (columns) =>
        sql`jsonb_build_object(${mapWithSeparator(
          columns,
          sql`, `,
          (c) => sql`${param(c)}::text, ${camelToSnakeFn(c)}`
        )})`,
      allColumnsJSON: (table) =>
        sql`(SELECT jsonb_object_agg(${snakeToCamelSQL(
          sql`${raw("key")}`
        )}, value) FROM jsonb_each(to_jsonb(${table}.*)))`,
    },
  };
...
export type NewConfig = Partial<
  Config | { nameTransforms: NameTransforms | boolean }
>; // special case for setting nameTransforms to true or false
...
export const setConfig = (newConfig: NewConfig) => {
  ...
  Object.assign(
    ...
    nameTransforms === false
      ? { nameTransforms: nullTransforms }
      : nameTransforms === true
      ? { nameTransforms: snakeCamelTransforms }
      : {}
  );
};
 ...
 */
export type RangeString<Bound extends string | number> = `${
  | "["
  | "("}${Bound},${Bound}${"]" | ")"}`;
 ...
 */
export function strict<FnIn, FnOut>(
  fn: (x: FnIn) => FnOut
): <T extends FnIn | null>(
  d: T
) => T extends FnIn ? Exclude<T, FnIn> | FnOut : T {
 ...
 */
export const toBuffer = strict((ba: ByteArrayString) =>
  Buffer.from(ba.slice(2), "hex")
);
 ...
 */
export function parent<T extends Column | undefined = Column | undefined>(
  x?: T
) {
...
>(literals: TemplateStringsArray, ...expressions: NoInfer<Interpolations>[]) {
  return new SQLFragment<RunResult, Constraint>(
...
> {
  ...
  runResultTransform: (qr: pg.QueryResult) => any = (qr) => {
    const { fromPgToTs } = config.nameTransforms.ts,
      { rows } = qr;

    if (fromPgToTs === noop) return rows;

    const namesMap = Object.fromEntries(
      qr.fields.map((f) => [f.name, fromPgToTs(f.name)])
    );
    for (let i = 0, len = rows.length; i < len; i++) {
      rows[i] = Object.fromEntries(
        Object.entries(rows[i]).map(([k, v]) => [namesMap[k], v])
      );
    }
