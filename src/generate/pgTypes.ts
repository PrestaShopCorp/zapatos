...
const baseTsTypeForBasePgType = (pgType: string, tsType: string, enums: EnumData, context: TypeContext, config: CompleteConfig) => {
  const
...
export const tsTypeForPgType = (pgType: string, tsType: string, enums: EnumData, context: TypeContext, config: CompleteConfig) => {
  // basic and enum types (enum names can begin with an underscore even if not an array)
  const baseTsType = baseTsTypeForBasePgType(pgType, tsType, enums, context, config);
  ...
  if (pgType.charAt(0) === '_') {
    const arrayTsType = baseTsTypeForBasePgType(pgType.slice(1), tsType, enums, context, config);
